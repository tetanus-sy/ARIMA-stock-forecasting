...
# Fitting ARIMA model
model = ARIMA(train_set, order=(0,1,1))
model_fit_0 = model.fit()
model_fit_0

# 预测
past = train_set['收盘价'].tolist()
predictions = []
    
for i in range(len(test_set)):
    model = ARIMA(past, order=(0,1,1))  # 可以替换为最佳参数组合
    model_fit = model.fit(start_params=model_fit_0.params)
    forecast_results = model_fit.forecast()
    pred = forecast_results
    predictions.append(pred)
    past.append(test_set.iloc[i, 0])  # 使用 .iloc[i, 0] 获取第i行的第0列值（假设第0列为收盘价列）
